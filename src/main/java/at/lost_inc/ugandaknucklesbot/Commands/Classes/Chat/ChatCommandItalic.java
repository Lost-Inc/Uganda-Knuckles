...
import at.lost_inc.ugandaknucklesbot.Util.ItalicConverter;
...
@Command(
        name = "italic",
        help = "Converts text, nicknames, and even channel names to italic text",
        categories = {BotCommand.ICategories.UTIL, BotCommand.ICategories.CHAT}
)
public final class ChatCommandItalic extends BotCommand {
    @Inject
    UtilsChat utilsChat;
    ...
    public void execute(@NotNull CommandParameter param) {
        ...
        if (utilsChat.isUserMention(param.args[0])) {
            ...
            try {
                member.modifyNickname(ItalicConverter.italicize(member.getEffectiveName())).queue();
                return;
        ...
        if (utilsChat.isChannelMention(param.args[0])) {
            ...
            try {
                param.message.getGuild().getGuildChannelById(channel.getIdLong()).getManager().setName(
                        ItalicConverter.italicize(channel.getName())
                ).queue();
                return;
            } catch (HierarchyException e) {
                hierarchyExceptionHandler(param.message.getChannel());
        ...
        utilsChat.sendInfo(param.message.getChannel(), ItalicConverter.italicize(String.join(" ", param.args)));
    }
